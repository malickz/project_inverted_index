...
public class Search {
	...
	public Map<Integer, Map<String, Integer>> search() throws Exception {
		...
		for (String word : this.qword_array) {
			...
			if (result.next()) {
				byte[] bytestream = result.getBytes(1);
		...
		/*
		 * TO-DO ... ... ResultSet rset = stmt.executeQuery(); if (rset.next())
		 * { byte[] bytestream = rset.getBytes(1); docid_freq_map =
		 * Delta.deltalist2sortedmap( VB.VBDECODE(bytestream) ); } ... TO-DO ...
		 */
	...
	public Map<Integer, Map<String, Double>> computeDocsTFIDFs() throws Exception {
		...
		for (Integer docid : docid_word_freq_map.keySet()) {
			docid_word_tfidf_map.put(docid, new HashMap<String, Double>()); // Initialize
			// for
			// docid
			...
			for (String word : docid_word_freq_map.get(docid).keySet()) {
				...
				// System.out.println(docid+":"+word+" " + "f="+f+", "+
				// "maxf="+maxf+", "+ "df="+df+", "+ "tfidf="+tfidf);
	...
	public Map<Integer, Double> score() throws Exception {
		...
		for (Integer docid : docid_word_tfidf_map.keySet()) {

			dotProductResult = dotproduct(docid_word_tfidf_map.get(docid), qword_tfidf_map);
			docid_score_map.put(docid,
					dotProductResult / (double) (qmagnitude * Index.retrieveDocInfo(docid).magnitude));
		}
	...
	public SortedMap<Double, Set<Integer>> sortResult(Map<Integer, Double> docid_score_map) throws Exception {
		SortedMap<Double, Set<Integer>> score_docidset_map = new TreeMap<Double, Set<Integer>>(
	...
	public static void main(String[] args) throws Exception {
		...
		Search mySearch = new Search("bahia");
